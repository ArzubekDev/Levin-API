...
interface Bolt {
  ...
  color: string;
  branches: Branch[];
  flash: number; // интенсивность вспышки фона
}
...
export function LightningBackground() {
  ...
  useEffect(() => {
    ...
    function generateBolt(target: HTMLCanvasElement): Bolt {
      ...
      // Генерируем zigzag путь
      for (let i = 0; i <= segmentsCount; i++) {
        const t = i / segmentsCount;
        const baseX = startX + (endX - startX) * t;
        // Джиттер уменьшается к низу, чтобы молния не разъезжалась слишком сильно
      ...
      return {
        ...
        color: Math.random() > 0.6 ? "#60a5fa" : "#818cf8", // blue-400 или indigo-400
        branches,
    ...
    function animate(timestamp: number) {
      ...
      // Trail-эффект: не чистим полностью, а рисуем полупрозрачный фон
      ctx.fillStyle = "rgba(2, 6, 23, 0.25)";
      ctx.fillRect(0, 0, canvas.width, canvas.height);

      // Вспышка фона от молнии
      bolts.forEach((bolt) => {
        if (bolt.life > 0.8) {
      ...
      // Создаём новую молнию каждые 2-5 секунд
      ...
      bolts = bolts.filter((bolt) => {
        bolt.life -= 0.011; // скорость затухания
    ...
    // Стартовый фон и первая молния сразу
